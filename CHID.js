class CHID {
  ...
  draw() {
    ...
    stroke(255, 0.5);
    let offsetX =
      (random(this.elementalBreakTick) - this.elementalBreakTick / 2) / 2;
    let offsetY =
      (random(this.elementalBreakTick) - this.elementalBreakTick / 2) / 2;
    fill(0);
    ...
    stroke(
      ...
      100,
      30,
      max(0.7 - (max(abs(HERO.lifeHighlightTime, 60)) - 60) / 240, 0.3)
    );
    ...
    strokeWeight(12);
    stroke(210, 100, 30, max(0.7 - abs(HERO.manaHighlightTime) / 30, 0.3));
    arc(mouseX + 4, mouseY, 200, 200, -PI / 2, PI / 2);
